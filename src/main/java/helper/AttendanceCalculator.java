...
public class AttendanceCalculator {
    ...
    public static List<DailyAttendance> calculate(List<AttendanceRaw> raws, List<Holiday> holidays, List<LeaveRequest> leaves, List<OTRequest> otRequests, List<Employee> employees) {
        ...
        for (Holiday h : holidays) {
            for (Employee emp : employees) {
                ...
                daily.setDate(new java.sql.Date(h.getDate().getTime()));
        ...
        Set<String> existingKeys = results.stream().map(d -> d.getEmployee().getEmpId() + "_" + d.getDate().toString()).collect(Collectors.toSet());
        ...
        //No log, no leave request
        Set<String> existingKeysAll = results.stream().map(d -> d.getEmployee().getEmpId() + "_" + d.getDate().toString()).collect(Collectors.toSet());
        Set<Date> allDates = raws.stream().map(r -> r.getDate()).collect(Collectors.toSet());
        for (Employee emp : employees) {
            for (Date date : allDates) {
                String key = emp.getEmpId() + "_" + date.toString();
